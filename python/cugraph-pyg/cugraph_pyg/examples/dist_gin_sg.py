...
class DistTensorGraphDataset(Dataset):
    ...
    def __init__(self, dist_edge_index, feature_store, device, edge_ptr, graph_indices=None):
        ...
        if len(self.graph_indices) < 1000:  # Only cache for smaller datasets
            self._cached_labels = self.feature_store["graph", "y", None][torch.tensor(self.graph_indices, device=self.device)]
    ...
    def __getitem__(self, idx):
        ...
        if self._cached_labels is not None:
            ...
            y = self.feature_store["graph", "y", None][torch.tensor([idx], device=self.device)]
        if y.dim() > 1:
            y = y.squeeze()
        ...
        node_to_local = torch.zeros(nodes_in_subgraph.max().item() + 1, dtype=torch.long, device=self.device)
        node_to_local[nodes_in_subgraph] = torch.arange(nodes_in_subgraph.size(0), device=self.device)
...
def custom_collate_fn(batch):
    ...
    if total_edges > 0:
        ...
        node_offsets = torch.cumsum(torch.cat([torch.zeros(1, dtype=torch.long, device=device), num_nodes_tensor[:-1]]), dim=0)
        ...
        for i, edge_index in enumerate(edge_index_list):
            if edge_index.size(0) > 0:
                ...
                edge_index_final[:, edge_offset:edge_offset + num_edges] = edge_index.t() + node_offsets[i]
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='ENZYMES',
                       choices=['MUTAG', 'ENZYMES', 'PROTEINS', 'COLLAB', 'IMDB-BINARY', 'REDDIT-BINARY'],
...
def get_dataset_stats(dataset_name):
    ...
    return stats.get(dataset_name, {'batch_size': 32, 'hidden_channels': 64, 'lr': 0.01})
...
def load_data(dataset_name, data_root, device, device_id):
    ...
    return (feature_store, dist_edge_index), edge_ptr, len(dataset), num_features, num_classes
...
def main():
    ...
    batch_size = args.batch_size if args.batch_size != 32 else dataset_stats['batch_size']
    hidden_channels = args.hidden_channels if args.hidden_channels != 64 else dataset_stats['hidden_channels']
    ...
    (feature_store, dist_edge_index), edge_ptr, num_graphs, num_features, num_classes = load_data(
    ...
    train_dataset = DistTensorGraphDataset(dist_edge_index, feature_store, device, edge_ptr, train_graph_indices)
    test_dataset = DistTensorGraphDataset(dist_edge_index, feature_store, device, edge_ptr, test_graph_indices)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=custom_collate_fn)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=custom_collate_fn)

    # Create model and optimizer
    model = GIN(num_features, hidden_channels, num_classes, args.num_layers, args.dropout).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f'  Train Time: {train_time:.4f}s, Test Time: {test_time:.4f}s, Total: {total_time:.4f}s')
    
    print(f'Training - Median: {torch.tensor(train_times).median():.4f}s, Average: {torch.tensor(train_times).mean():.4f}s')
    print(f'Testing  - Median: {torch.tensor(test_times).median():.4f}s, Average: {torch.tensor(test_times).mean():.4f}s')
    print(f'Total    - Median: {torch.tensor(total_times).median():.4f}s, Average: {torch.tensor(total_times).mean():.4f}s')
