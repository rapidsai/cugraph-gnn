...
def load_library():
    ...
    libs_to_return = []
    for soname in ["libwholegraph.so"]:
        libwholegraph_lib = None
        if prefer_system_installation:
            # Prefer a system library if one is present to
            # avoid clobbering symbols that other packages might expect, but if no
            # other library is present use the one in the wheel.
            try:
                libwholegraph_lib = _load_system_installation(soname)
            except OSError:
                libwholegraph_lib = _load_wheel_installation(soname)
        else:
            ...
            try:
                ...
                pass
        if libwholegraph_lib:
            libs_to_return.append(libwholegraph_lib)
